...
public:
    ...
    size_type copyIfNotEquivalentTo(const Map &source, const Key &key)
    {
        ...
        size_type result = 0;
        const auto &keyCompare = source.key_comp();  //operator <
        ...
        std::remove_copy_if(source.cbegin(), source.cend(), std::inserter(m, m.end()), filter);
    }
    ...
    Key key(const T &value, const Key &defaultKey) const {
        auto i = std::find_if(m.cbegin(), m.cend(), valueIsEqualTo(value));
        if (i != m.cend()) {
            return i->first;
        }
        return defaultKey;
    ...
    QList<T> values() const
    {
        ...
        std::transform(m.cbegin(), m.cend(), std::back_inserter(result), extractValue);
        return result;
    }
...
public:
    ...
    bool isSharedWith(const QMap<Key, T> &other) noexcept
    {
        return d == other.d;
    }
    ...
    T take(const Key &key)
    {
        ...
        auto i = d->m.find(key);
        if (i != d->m.end()) {
            T result(std::move(i->second));  //std::move提升效率
            d->m.erase(i);
            return result;
        }
        return T();
    ...
    class iterator  //内置使用了std::map的iterator
    {
    ...
    class const_iterator
    {
        ...
        explicit(false) const_iterator(typename Map::const_iterator it) : i(it) {}
    ...
    public:
        ...
        friend const_iterator operator+(const_iterator it, int j) { return std::next(it, j); }
        friend const_iterator operator-(const_iterator it, int j) { return std::prev(it, j); }
    ...
    const const_iterator lowerBound(const Key &key) const {
        if (!d) {
            return const_iterator();
        }
    ...
    void insert(QMap<Key, T> &&map) {
        ...
        if (map.d.isShared()) {
            insert(map);  //调用到了insert(const QMap<Key, T> &)
            return;
        }
        detach();
...
public:
        ...
        : d(other.isEmpty() ? nullptr : new MapData)
    {
        if (d) {
            Q_ASSERT(other.d);
            d->m.insert(other.d->m.begin(), other.d->m.end());
        }
    }
    ...
    size_type size() const { return d ? size_type(d->m.size()) : size_type(0); }
    bool isEmpty() const { return d ? d->m.empty() : true; }
    bool isDetached() const { return d ? !d.isShared() : false; }
    void detach() {
        if (d) {
            d.detach();
        }
    ...
    QList<Key> uniqueKeys() const {
        ...
        result.reserve(size());
        std::unique_copy(keyBegin(), keyEnd(), std::back_inserter(result));
        result.shrink_to_fit();
        return result;
    }
    ...
    size_type count(const Key &key, const T &value) const {
        ...
        return size_type(std::count_if(range.first, range.second, MapData::valueIsEqualTo(value)));
    }
    ...
    iterator find(const Key &key, const T &value)
    {
        detach();
        auto range = d->m.equal_range(key);
        auto i = std::find_if(range.first, range.second, MapData::valueIsEqualTo(value));
        if (i != range.second) {
            return iterator(i);
        }
        return iterator(d->m.end());
