//
// Created by Yujie Zhao on 2023/10/11.
//
#include <QObject>
#include <QTest>
#include <QHash>
...
inline size_t qHash(IdentityTracker key) {
    return qHash(key.value);
}

inline bool operator==(IdentityTracker lhs, IdentityTracker rhs) {
...
public:
    ...
    void registerInvokeMethods() override {
        REGISTER_OBJECT_INVOKE_METHOD(insert1)
        REGISTER_OBJECT_INVOKE_METHOD(erase)
        REGISTER_OBJECT_INVOKE_METHOD(erase_edge_case)
        REGISTER_OBJECT_INVOKE_METHOD(key)
        REGISTER_OBJECT_INVOKE_METHOD(keys)
        REGISTER_OBJECT_INVOKE_METHOD(swap)
        REGISTER_OBJECT_INVOKE_METHOD(count)
        REGISTER_OBJECT_INVOKE_METHOD(clear)
        REGISTER_OBJECT_INVOKE_METHOD(empty)
        REGISTER_OBJECT_INVOKE_METHOD(find)
        REGISTER_OBJECT_INVOKE_METHOD(constFind)
        REGISTER_OBJECT_INVOKE_METHOD(contains)
        REGISTER_OBJECT_INVOKE_METHOD(qhash)
        REGISTER_OBJECT_INVOKE_METHOD(take)
        REGISTER_OBJECT_INVOKE_METHOD(operator_eq)
        REGISTER_OBJECT_INVOKE_METHOD(rehash_isnt_quadratic)
        REGISTER_OBJECT_INVOKE_METHOD(dont_need_default_constructor)
        REGISTER_OBJECT_INVOKE_METHOD(qmultihash_specific)
        REGISTER_OBJECT_INVOKE_METHOD(qmultihash_qhash_rvalue_ref_ctor)
        REGISTER_OBJECT_INVOKE_METHOD(qmultihash_qhash_rvalue_ref_unite)
        REGISTER_OBJECT_INVOKE_METHOD(qmultihashUnite)

        REGISTER_OBJECT_INVOKE_METHOD(compare)
        REGISTER_OBJECT_INVOKE_METHOD(compare2)
        REGISTER_OBJECT_INVOKE_METHOD(iterators)
        REGISTER_OBJECT_INVOKE_METHOD(multihashIterators)
        REGISTER_OBJECT_INVOKE_METHOD(iteratorsInEmptyHash)
        REGISTER_OBJECT_INVOKE_METHOD(keyIterator)
        REGISTER_OBJECT_INVOKE_METHOD(multihashKeyIterator)
        REGISTER_OBJECT_INVOKE_METHOD(keyValueIterator)
        REGISTER_OBJECT_INVOKE_METHOD(multihashKeyValueIterator)
        REGISTER_OBJECT_INVOKE_METHOD(keyValueIteratorInEmptyHash)
        REGISTER_OBJECT_INVOKE_METHOD(keys_values_uniqueKeys)

        REGISTER_OBJECT_INVOKE_METHOD(const_shared_null)
        REGISTER_OBJECT_INVOKE_METHOD(twoArguments_qHash)
        REGISTER_OBJECT_INVOKE_METHOD(initializerList)
        REGISTER_OBJECT_INVOKE_METHOD(eraseValidIteratorOnSharedHash)
        REGISTER_OBJECT_INVOKE_METHOD(equal_range)
        REGISTER_OBJECT_INVOKE_METHOD(insert_hash)
        REGISTER_OBJECT_INVOKE_METHOD(emplace)
        REGISTER_OBJECT_INVOKE_METHOD(badHashFunction)
        REGISTER_OBJECT_INVOKE_METHOD(hashOfHash)
        REGISTER_OBJECT_INVOKE_METHOD(stdHash)
        REGISTER_OBJECT_INVOKE_METHOD(countInEmptyHash)
        REGISTER_OBJECT_INVOKE_METHOD(removeInEmptyHash)
        REGISTER_OBJECT_INVOKE_METHOD(valueInEmptyHash)
        REGISTER_OBJECT_INVOKE_METHOD(fineTuningInEmptyHash)
    }
...
void tst_QHash::keys() {
    {
        ...
        QCOMPARE(sorted(hash.keys()), QList<QString>({ "key1", "key2", "key3" }));
        QCOMPARE(sorted(hash.keys(1)), QList<QString>({ "key1", "key3" }));
    }
    {
        ...
        QCOMPARE(sorted(hash.keys()), QList<QString>({ "key1", "key1", "key2" }));
        QCOMPARE(sorted(hash.keys(1)), QList<QString>({ "key1", "key2" }));
    }
}
...
static QString stringArg(const QString &s, int i) {
    return s + i;
};
...
void tst_QHash::find()
{
    ...
    for (i = 3; i < 10; ++i) {
        compareString = stringArg(testString, i);
...
void tst_QHash::dont_need_default_constructor()
{
    ...
    //这句代码会报错，value函数中使用了const T &defaultValue = T()， 而Bar没有无参构造
//    auto it = hash1.value(1);
}
...
void tst_QHash::qmultihash_qhash_rvalue_ref_ctor()
{
    ...
    // QHash is detached
    {
        ...
        MyClass::moves = 0;
        QHash<int, MyClass> hash;
        hash.emplace(0, "a");  //"a"被当作参数传递进去原地构造了
        hash.emplace(1, "b");
        QMultiHash<int, MyClass> multiHash(std::move(hash));
...
void tst_QHash::compare()
{
    ...
    for (i = 0; i < 1000; ++i)
        hash1.insert(i, stringArg(testString, i));

    for (--i; i >= 0; --i)
        hash2.insert(i, stringArg(testString, i));
...
void tst_QHash::iterators()
{
    ...
    for (i = 1; i < 100; ++i)
        hash.insert(i, stringArg(testString, i));
...
void tst_QHash::multihashIterators()
{
    ...
    auto testStringArgFunc=[](int i, int j)->QString {
        return QString("Teststring ") + i + "-" + j;
    };
    ...
    for (i = 0; i < 10; ++i) {
        for (int j = 0; j < 5; ++j)
            hash.insert(i, testStringArgFunc(i, j));
    }
