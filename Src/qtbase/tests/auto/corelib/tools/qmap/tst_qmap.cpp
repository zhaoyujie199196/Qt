...
public:
    void registerInvokeMethods() {
        REGISTER_OBJECT_INVOKE_METHOD(init)
        REGISTER_OBJECT_INVOKE_METHOD(ctor)
        REGISTER_OBJECT_INVOKE_METHOD(count)
        REGISTER_OBJECT_INVOKE_METHOD(clear)
        REGISTER_OBJECT_INVOKE_METHOD(beginEnd)
        REGISTER_OBJECT_INVOKE_METHOD(firstLast)
        ...
        REGISTER_OBJECT_INVOKE_METHOD(lowerUpperBound)
        REGISTER_OBJECT_INVOKE_METHOD(mergeCompare)
        REGISTER_OBJECT_INVOKE_METHOD(take)
        REGISTER_OBJECT_INVOKE_METHOD(iterators)
        REGISTER_OBJECT_INVOKE_METHOD(multimapIterators)
        REGISTER_OBJECT_INVOKE_METHOD(iteratorsInEmptyMap)
        REGISTER_OBJECT_INVOKE_METHOD(keyIterator)
        REGISTER_OBJECT_INVOKE_METHOD(multimapKeyIterator)
        REGISTER_OBJECT_INVOKE_METHOD(keyValueIterator)
        REGISTER_OBJECT_INVOKE_METHOD(multimapKeyValueIterator)
        REGISTER_OBJECT_INVOKE_METHOD(keyValueIteratorInEmptyMap)
        REGISTER_OBJECT_INVOKE_METHOD(keys_values_uniqueKeys)
        REGISTER_OBJECT_INVOKE_METHOD(qmultimap_specific)
        ...
        REGISTER_OBJECT_INVOKE_METHOD(insertMap)
        REGISTER_OBJECT_INVOKE_METHOD(checkMostLeftNode)
        REGISTER_OBJECT_INVOKE_METHOD(initializerList)
        REGISTER_OBJECT_INVOKE_METHOD(testInsertWithHint)
        REGISTER_OBJECT_INVOKE_METHOD(testInsertMultiWithHint)
        REGISTER_OBJECT_INVOKE_METHOD(eraseValidIteratorOnSharedMap)
        REGISTER_OBJECT_INVOKE_METHOD(removeElementsInMap)
        REGISTER_OBJECT_INVOKE_METHOD(toStdMap)
    }
...
{
    {
        ...
        QCOMPARE(map.count(QString("key")), 0);
        QCOMPARE(map.size(), 0);
...
void tst_QMap::find()
{
    ...
    for (i = 3; i < 10; ++i) {
        compareString = testString + QString::number(i);
...
void tst_QMap::iterators()
{
    ...
    for(i = 1; i < 100; ++i) {
        auto t = "Teststring ";
        map.insert(i, t + QString::number(i));
    }
    ...
    for(stlIt = map.begin(), i = 1; stlIt != map.end(); ++stlIt, ++i) {
        bool r = stlIt.value() == ("Teststring " + QString::number(i));
        QVERIFY(r);
    }
    QCOMPARE(i, 100);
    ...
    for(stlIt = map.begin(), i = 1; stlIt != map.end(); ++stlIt, ++i) {
        auto r = stlIt.value() == "Teststring " + QString::number(i);
        QVERIFY(r);
    }
    QCOMPARE(i, 100);
    QT_WARNING_POP
    ...
    for(cstlIt = map.constBegin(), i = 1; cstlIt != map.constEnd(); ++cstlIt, ++i) {
        auto r = cstlIt.value() == "Teststring " + QString::number(i);
        QVERIFY(r);
    }
    QCOMPARE(i, 100);
    ...
    while(javaIt.hasNext()) {
        ...
        javaIt.next();
        auto v = javaIt.value();
        auto r = v == ("Teststring " + QString::number(i));
        QVERIFY(r);
    }
    ...
    while(javaIt.hasPrevious()) {
        --i;
        javaIt.previous();
        auto r = javaIt.value() == "Teststring " + QString::number(i);
        QVERIFY(r);
    }
...
void tst_QMap::multimapIterators()
{
    ...
    auto createStrFunc=[testString](int i, int j)->QString {
        auto r = testString;
        r.replace("%1", QString::number(i));
        r.replace("%2", QString::number(j));
        return r;
    };
...
{
    {
        ...
        map.insert("one", 1);
        QCOMPARE(map.keys(), QList<QString>({ "one" }));
        QCOMPARE(map.keys(1), QList<QString>({ "one" }));
        QCOMPARE(map.values(), QList<int>({ 1 }));

        map.insert("two", 2);
        QCOMPARE(map.keys(), QList<QString>({ "one", "two" }));
        QCOMPARE(map.keys(1), QList<QString>({ "one" }));
        QCOMPARE(map.values(), QList<int>({ 1, 2 }));

        map.insert("three", 2);
        QCOMPARE(map.keys(), QList<QString>({ "one", "three", "two" }));
        QCOMPARE(map.keys(2), QList<QString>({ "three", "two" }));
        QCOMPARE(map.values(), QList<int>({ 1, 2, 2 }));

        map.insert("one", 0);
        QCOMPARE(map.keys(), QList<QString>({ "one", "three", "two" }));
        QCOMPARE(map.keys(1), QList<QString>());
        QCOMPARE(map.keys(0), QList<QString>({ "one" }));
        QCOMPARE(map.keys(2), QList<QString>({ "three", "two" }));
