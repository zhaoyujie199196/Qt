...
//相同的内存结构，通过void *作为转换中介，取内部的非public数据
template<typename T>
static inline QtSharedPointer::ExternalRefCountData *refCountData(const QSharedPointer<T> &b)
{
    // access d-pointer:
    struct Dummy {
        void* value;
...
void tst_QSharedPointer::differentPointers() {
    {
        ...
        if (*reinterpret_cast<quintptr *>(&aData) == *reinterpret_cast<quintptr *>(&aBase)) {
            Q_ASSERT(false);
            std::cout<<("Something went very wrong -- we couldn't create two different pointers to the same object")<<std::endl;
        }
...
QTEST_APPLESS_MAIN(tst_QSharedPointer)